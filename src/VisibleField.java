...
public class VisibleField {
   ...
   public VisibleField(MineField mineField) {
      ...
      for (int[] statusRow : status) {
         Arrays.fill(statusRow, COVERED);
      }
   ...
   */     
   ...
   /**
      ...
      If the square isn't adjacent to a mine, it also recursively uncovers all the squares
      in the neighboring area in the same way.
      @param row of the square
      @param col of the square
      PRE: getMineField().inRange(row, col)
    */
   private void uncoverArea(int row, int col) {
      if (mineField.hasMine(row, col) || status[row][col] == MINE_GUESS || isUncovered(row, col)) {
         return;
      }

      int numAdjacentMines = mineField.numAdjacentMines(row, col);
      status[row][col] = numAdjacentMines;
